...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "smarthire_ai_secret_2024")

# Use /tmp for writable storage on cloud platforms (Render, Railway, etc.)
...
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyD7gSGcURKEtd6g8EMCMN2a3XOxhq5V7J8")
    ...
    for i in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[Gemini API] Returned status {response.status_code}. Retrying in {delay}s...")
                time.sleep(delay)
                delay *= 2.0
            else:
                response.raise_for_status()
        except Exception as e:
            print(f"[Gemini API] Error on attempt {i+1}: {e}")
            if i < retries - 1:
                time.sleep(delay)
                delay *= 2.0
            else:
...
def local_resume_match_fallback(jd, resume_text):
    jd_skills = ai_extract_skills(jd)
    resume_skills = ai_extract_skills(resume_text)
    
    matched_set = set(s.lower() for s in resume_skills)
    ...
    if jd_skills:
        ...
        # Fallback if no skills in JD, check matches with common platform skills
        matched_platform = [s for s in resume_skills if s in ["Python","Java","JavaScript","C++","C","React","Node.js","SQL","Git"]]
        score = min(90, max(35, len(matched_platform) * 10))
        matched_skills = matched_platform[:3]
        missing_skills = ["Docker", "AWS", "FastAPI"]
        
    verdict = "Excellent Match" if score >= 75 else "Good Match" if score >= 50 else "Needs Improvement"
    summary = f"Based on skill extraction analysis, your resume matches the job description. Identified {len(matched_skills)} matching key skills."
...
@app.route("/ai/resume_match", methods=["POST"])
def ai_resume_match():
    import secrets
    ...
    if f and f.filename != "":
        ext = f.filename.rsplit(".", 1)[-1].lower()
        temp_filename = f"temp_match_{session.get('uid', 0)}_{secrets.token_hex(4)}.{ext}"
        ...
        if not row or not row["resume"]:
            return jsonify({"error": "No resume uploaded yet. Please upload a resume."}), 400
        filepath = os.path.join(UPLOAD_FOLDER, row["resume"])
        if not os.path.exists(filepath):
            return jsonify({"error": "Resume file not found on server. Please upload it."}), 400
        resume_text = parse_resume_text(filepath)
        
    if not resume_text or not resume_text.strip():
        return jsonify({"error": "Could not extract text from the resume. Please ensure it is a valid PDF, DOCX, or TXT file."}), 400
    ...
    if not response_text:
        ...
        print("[Gemini API] Failed to get response, using local fallback matcher.")
        result = local_resume_match_fallback(jd, resume_text)
    ...
    try:
        ...
        # Fallback to local matching logic on JSON parse failure
        print(f"[Gemini API] Failed to parse response JSON: {e}, using local fallback matcher.")
        result = local_resume_match_fallback(jd, resume_text)
        return jsonify(result)
...
def chatbot():
    ...
    prompt = (
        f"You are 'SmartHire AI', a helpful campus recruitment assistant on the SmartHire AI platform.\n"
        f"The logged-in user name is {user_name} and their role is {user_role}.\n"
        f"User message: {msg}\n"
        f"Provide a helpful, friendly, and concise response (maximum 1-3 sentences). Do not use markdown headers."
    )
    
    reply = call_gemini(prompt)
    if not reply:
        print("[Gemini API] Chatbot failed, using local rule-based response.")
        msg_lower = msg.lower()
        if any(w in msg_lower for w in ["interview","schedule","when"]):
        ...
        elif any(w in msg_lower for w in ["offer","letter"]):
            ...
            reply = f"Hi {user_name}! 👋 I'm SmartHire AI. Ask me about jobs, eligibility, or your applications."
        else:
            reply = "I'm having trouble connecting to my AI brain right now. I can help you with: job eligibility, application status, AI match scores, interview prep, and skill tips!"
